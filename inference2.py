...
def inference(config: DINetInferenceOptions):
    ...
    video_frame_dir = config.source_video_path.replace(".mp4", "")
    if not os.path.exists(video_frame_dir):
        os.mkdir(video_frame_dir)
    video_size = extract_frames_from_video(config.source_video_path, video_frame_dir)
    end_time = time.time()
    logging.info(f"Frames extraction took {end_time - start_time:.2f} sec.")

    # extract audio features using Hubert Model from Pytorch
    logging.info("extracting audio speech features from : %s", config.driving_audio_path)
    start_time = time.time()
    ds_feature = feature_extractor.compute_audio_feature(
        config.driving_audio_path
    ...
    logging.info(
        "loading facial landmarks from : %s", config.source_openface_landmark_path
    )
    if not os.path.exists(config.source_openface_landmark_path):
        raise ValueError(
            "wrong facial landmark path :%s", config.source_openface_landmark_path
        )
    video_landmark_data = load_landmark_openface(
        config.source_openface_landmark_path
    ).astype(np.int64)
    ...
    ref_img_list = []
    resize_w = int(config.mouth_region_size + config.mouth_region_size // 4)
    resize_h = int((config.mouth_region_size // 2) * 3 + config.mouth_region_size // 8)
    ...
    logging.info("loading pretrained model from: %s", config.pretrained_clip_DINet_path)
    model = DINet(config.source_channel, config.ref_channel, config.audio_channel).cuda()
    if not os.path.exists(config.pretrained_clip_DINet_path):
        raise ValueError(
            "wrong path of pretrained model weight: %s", config.pretrained_clip_DINet_path
        )
    state_dict = torch.load(config.pretrained_clip_DINet_path)["state_dict"]["net_g"]
    ...
    if not os.path.exists(config.res_video_dir):
        os.mkdir(config.res_video_dir)
    res_video_path = os.path.join(
        config.res_video_dir,
        os.path.basename(config.source_video_path)[:-4] + "_facial_dubbing.mp4",
    )
    ...
    for clip_end_index in range(5, pad_length, 1):
        ...
        crop_frame_data[
            config.mouth_region_size // 2 : config.mouth_region_size // 2
            + config.mouth_region_size,
            config.mouth_region_size // 8 : config.mouth_region_size // 8
            + config.mouth_region_size,
            :,
        ] = 0
    ...
    #     res_video_path, config.driving_audio_path, video_add_audio_path
    # )
